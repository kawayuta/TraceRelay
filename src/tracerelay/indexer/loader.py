...
class TaskRuntimeProjector:
    ...
    def apply_schema(self, connection: Any) -> None:
        with connection.cursor() as cursor:
            # Parallel container startup can hit the schema DDL at the same time.
            # IF NOT EXISTS is not enough to prevent all PostgreSQL catalog races,
            # so serialize schema application with a transaction-scoped advisory lock.
            cursor.execute("SELECT pg_advisory_xact_lock(%s, %s)", (48251, 3003))
