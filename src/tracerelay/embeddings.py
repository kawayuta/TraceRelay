...
def embedder_from_env() -> TextEmbedder:
    explicit_provider = os.getenv("TRACERELAY_EMBEDDING_PROVIDER", "").strip().casefold()
    ...
    if provider in {"anthropic", "claude"}:
        raise EmbeddingError(
            ...
            "Use openai, gemini, ollama, lmstudio, or hash for TRACERELAY_EMBEDDING_PROVIDER."
        )
    if provider == "openai":
        api_key = os.getenv("TRACERELAY_OPENAI_API_KEY", "").strip()
        model = os.getenv("TRACERELAY_OPENAI_EMBEDDING_MODEL", "").strip() or "text-embedding-3-small"
        base_url = os.getenv("TRACERELAY_OPENAI_BASE_URL", "https://api.openai.com").strip()
        if not api_key:
    ...
    if provider == "gemini":
        ...
        model = os.getenv("TRACERELAY_GEMINI_EMBEDDING_MODEL", "").strip() or "gemini-embedding-001"
        base_url = os.getenv("TRACERELAY_GEMINI_BASE_URL", "https://generativelanguage.googleapis.com").strip()
        if not api_key:
...
def _infer_embedding_provider_from_env() -> str:
    provider = os.getenv("TRACERELAY_EMBEDDING_PROVIDER", "").strip().casefold()
    if provider in {"lmstudio", "ollama", "openai", "gemini", "anthropic", "claude", "hash"}:
        return provider
    llm_provider = os.getenv("TRACERELAY_LLM_PROVIDER", "").strip().casefold()
    if llm_provider in {"lmstudio", "ollama"}:
        return llm_provider
    if os.getenv("TRACERELAY_OPENAI_API_KEY"):
        return "openai"
    if os.getenv("TRACERELAY_GEMINI_API_KEY"):
        return "gemini"
    if os.getenv("TRACERELAY_LM_STUDIO_BASE_URL"):
        return "lmstudio"
    if os.getenv("TRACERELAY_OLLAMA_BASE_URL"):
        return "ollama"
    return "hash"
