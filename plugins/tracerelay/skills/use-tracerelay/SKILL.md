...
4. Use `task_trace` immediately after a run when you need to explain how the runtime chose re-extract versus schema evolution.
5. Use `schema_status` when the user asks which keys or relations were added.
6. Use `artifact_read` only when you need the raw lineage artifacts.
