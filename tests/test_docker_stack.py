...
def test_db_module_help() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "tracerelay.db", "--help"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        check=False,
    ...
    assert "Apply TraceRelay PostgreSQL schema" in result.stdout
...
def test_docker_start_web_script_declares_schema_apply_and_web_server() -> None:
    ...
    assert "trr db apply-schema" in script
    assert "--reindex" in script
    assert "trr web serve" in script
...
def test_docker_start_mcp_script_declares_schema_apply_and_mcp_server() -> None:
    ...
    assert "trr db apply-schema" in script
    assert "--reindex" in script
    assert "trr mcp serve" in script
    assert "TRACERELAY_MCP_TRANSPORT" in script
